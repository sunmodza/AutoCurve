            ...
            if x_ind is not None:
                if rd.randint(0, 100) <= prob_of_use_x:
                    ...
                    # case nas = n ** x ; x = log n a
                        ...
                        x = np.mean(
                            (np.log(ans.num)/np.log(subset[n_ind].num)))
                    subset[x_ind] = NumberWithText(x, text=str(x))
            vas = subset[0] ** subset[1]
            # print(vas)
...
class IQ180_Solution:
    ...
    def clean_text(self, eq_cleaned):
        ...
        for i in range(len(self.init_quest)-1):
            ...
            # print(element)
            eq_cleaned = eq_cleaned.replace(
                element, self.get_value_of_value(element))
        # print(eq_cleaned)
        # print(eq_cleaned)
            ...
            for quest in self.packing:
                ...
                for m in ms:
                    ...
                    try:
                        ...
                        continue
                    MSE = find_dist(equal_to.num, self.ans.num) / \
                        self.ans.num.shape[0]
                    ...
                    if MSE < shortest and isinstance(equal_to.num, np.ndarray):
                        ...
                        plt.plot(self.key_by_order["V"],self.ans.num,"or")
                        plt.plot(self.key_by_order["V"],equal_to.num,"b")
                        # plt.show()
                        plt.pause(0.5)
                        ...
                        print(MSE)
                        # print(shortest)
                    # and (self.not_use_all or len(remain) == 0):
                    if (MSE < accept_error):
                        ...
                        if True:
                            ...
                            self.all_solution_equation.append(
                                equal_to.num.tolist())
                            # print(MSE,len(self.packing),equal_to.opr_count)
                    rd.shuffle(self.packing)
                    # print(MSE,self.wpe,rework)
                    if len(self.packing) > self.reset_param:
                        best_variable = best_variable[-int(
                            len(self.packing)/self.keep_best_variable_params):]
...
#tool.set_start(ele, ans, sol_nums=1, not_use_all=False,
               #llow_fac=False, reset_params=300, prob_of_use_x=90)
